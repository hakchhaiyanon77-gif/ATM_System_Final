...
public class ATM {
    ...
    public ATM() {
        try {
            Account savings = new SavingsAccount("1001", "1234", 1000, 500, 0.01, 10);
            Account checking = new CheckingAccount("1002", "2345", 2000, 1000, 200);
    ...
    public boolean login(String cardNumber, String pin) throws ATMException {
        ...
        if (card == null) throw new AccountNotFoundException("Card/account number not found.");
    ...
    public void checkBalance() throws ATMException {
        requireLogin();
        System.out.printf(Locale.US, "Balance: $%.2f%n", currentAccount.getBalance());
        if (currentAccount instanceof CheckingAccount) {
            CheckingAccount checking = (CheckingAccount) currentAccount;
            System.out.printf(Locale.US, "Available including overdraft: $%.2f%n", checking.getAvailableBalance());
        }
        currentAccount.addTransaction(new Transaction(TransactionType.BALANCE_INQUIRY,
                0, currentAccount.getBalance()));
    }
    ...
    public void transfer(String accountNo, double amount) throws ATMException {
        ...
        if (receiver == null) throw new AccountNotFoundException("Receiving account not found.");
        if (cards.get(accountNo).isLocked()) throw new ATMException("Receiving account is locked.");
    ...
    public void showHistory() throws ATMException {
        requireLogin();
        if (currentAccount.getTransaction().isEmpty()) System.out.println("No transactions yet.");
        for (Transaction t : currentAccount.getTransaction()) {
            System.out.printf(Locale.US, "%s | %s | $%.2f | Balance: $%.2f%n",
    ...
    public void changePIN(String oldPin, String newPin) throws ATMException {
        ...
        if (!currentCard.validatePIN(oldPin)) {
            ...
            if (locked) logout();
            throw new InvalidPinException(locked ? "Card/account locked. Session ended." : "Old PIN is incorrect.");
        }
    ...
    public void start() {
        try {
            while (true) {
                ...
                if (choice.equals("0")) { System.out.println("Goodbye."); return; }
                if (!choice.equals("1")) { System.out.println("Choose 1 or 0."); continue; }
                try {
                    login(read("Card/account number: "), read("PIN: "));
                    System.out.println("Welcome, account " + currentAccount.getAccountNumber()
                    ...
                    while (isAuthenticated) {
                        ...
                        try {
                            switch (read("Choose: ")) {
                                ...
                                case "2": deposit(readAmount("Deposit amount: ")); break;
                                case "3": withdraw(readAmount("Withdrawal amount: ")); break;
                                case "4":
                                    String target = read("Receiver account number: ");
                                    transfer(target, readAmount("Transfer amount: "));
                                    break;
                                case "5": showHistory(); break;
                                case "6": changePIN(read("Old PIN: "), read("New PIN: ")); break;
                                case "7":
                                    if (!(currentAccount instanceof SavingsAccount)) {
                                        throw new ATMException("Interest is only available for savings.");
                                    }
                                    ((SavingsAccount) currentAccount).applyInterest();
                                    System.out.println("Monthly interest applied (1%).");
                                    break;
                                case "0": logout(); System.out.println("Logged out."); break;
                                default: System.out.println("Choose a number from the menu.");
                            }
                        } catch (ATMException e) { System.out.println(e.getMessage()); }
                    }
                } catch (ATMException e) { System.out.println(e.getMessage()); }
            }
